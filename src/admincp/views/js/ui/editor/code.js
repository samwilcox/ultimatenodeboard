...
UNB_ADMINCP.on('ui.init', () => {
    ...
    $(document).on('click', '[data-tool="code"]', function (e) {
        ...
        if (!editorKey) {
            UNB_ADMINCP.log.warn('Could not determine the editorKey for code tool.', 'UNBEditor');
            return;
        }
        ...
        if (!$modal.length) {
            UNB_ADMINCP.log.warn(`No code modal found for editor: ${editorKey}.`, 'UNBEditor');
            return;
        }
...
const setupCodeModal = ($modal) => {
    ...
    $insert.on('click', function (e) {
        ...
        if (!editor) {
            UNB_ADMINCP.log.warn(`No editor found for editorKey: ${editorKey}.`, 'UNBEditor');
            return;
        }
        ...
        if (language === 'autodetect') {
            try {
                const res = UNB_ADMINCP.editor.lowLight?.highlightAuto(codeText);
