...
UNB.on('ui.init', () => {
    ...
    UNB.modal = UNB.modal || {};
    UNB.modal.open = openModal;
    UNB.modal.close = closeModal;
    UNB.modal.closeAll = () => activeModal && closeModal(activeModal);
    ...
    $(document).on('click', '[data-unb-modal-open]', function (e) {
        ...
        const $editorWrap = $(this).closest('.unb-editor');

        if ($editorWrap.length) {
            const editorKey = $editorWrap.data('unb-editor-key') || null;
            $modal.data('editor-key', editorKey);
        } else {
            // Ensure global modals do not retain stale editor key
            $modal.removeData('editor-key');
        }
